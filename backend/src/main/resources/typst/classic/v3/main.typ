// Classic v3 — built-in read-only template. Presentation only; all content comes from the
...
#let render-skill-section(section) = {
  let entries = section.entries
  let cells = entries.map(entry => render-skill-cell(entry))
  if entries.len() > 1 {
    block(breakable: false, {
      section-title(section.title)
      grid(columns: (1fr, 1fr), gutter: 0.8em, ..cells.slice(0, 2))
    })
    if entries.len() > 2 {
      grid(columns: (1fr, 1fr), gutter: 0.8em, ..cells.slice(2))
    }
  } else {
    block(breakable: false, {
      section-title(section.title)
      render-skill-cell(entries.at(0))
    })
  }
  v(0.1em)
}
