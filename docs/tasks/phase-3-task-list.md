...
在 v2.9 简历解析质量优化完成后，基于已有的简历解析、目标岗位解析、AI 匹配分析、岗位优化建议和局部改写结果，生成一份面向目标岗位的“岗位优化报告”。

v2.10 不新增复杂 AI 能力，不生成完整定制版简历，不导出 DOCX / PDF，不自动修改用户原始简历文件。
本版本重点解决用户完成匹配和建议后“最终拿到什么”的问题，让系统形成更清晰的使用闭环和演示闭环。

v2.10 完成后应满足：

- 用户可以基于一份简历和一个目标岗位查看岗位优化报告。
- 报告聚合展示匹配分数、强匹配项、弱匹配项、缺失技能、优化建议、局部改写建议和风险提示。
- 报告只基于已有系统结果生成，不编造新的经历、技能、证书、奖项或量化指标。
- 报告可以用于用户手动修改简历，也可以用于面试演示截图。
- 前端可以在“匹配与优化”页面查看报告。
- 文档中形成一条可演示的完整用户流程。
- 本轮结果记录到 `docs/iteration-log/v2.10-optimization-report.md`。

说明：

- v2.10 不做完整简历生成。
- v2.10 不做 DOCX / PDF 导出。
- v2.10 不自动写回原始简历。
- v2.10 不新增新的岗位匹配算法。
- v2.10 不新增新的 RAG / Embedding 能力。
- v2.10 不做多轮对话。
- v2.10 不做前端大规模 UI 重构。
- v2.10 不做部署、域名、HTTPS、服务器上线。
- v2.10 的重点是聚合已有结果，形成“可查看、可复制、可演示”的岗位优化报告。
...
- 明确岗位优化报告的数据结构。
- 明确报告只聚合已有结果，不重新生成大段 AI 内容。
- 明确报告中哪些字段来自 AI 匹配、哪些字段来自优化建议、哪些字段来自局部改写。

任务：

- [ ] 设计 `JobOptimizationReportVO`。
- [ ] 报告包含 `resumeId`。
- [ ] 报告包含 `resumeName`。
- [ ] 报告包含 `jobDescriptionId`。
- [ ] 报告包含 `jobTitle`。
- [ ] 报告包含 `matchScore`。
- [ ] 报告包含 `matchLevel`，例如 HIGH、MEDIUM、LOW。
- [ ] 报告包含 `strongMatches`。
- [ ] 报告包含 `weakMatches`。
- [ ] 报告包含 `missingSkills`。
- [ ] 报告包含 `riskTips`。
- [ ] 报告包含 `suggestionSummary`。
- [ ] 报告包含 `highPrioritySuggestions`。
- [ ] 报告包含 `mediumPrioritySuggestions`。
- [ ] 报告包含 `lowPrioritySuggestions`。
- [ ] 报告包含 `rewriteSuggestions`。
- [ ] 报告包含 `acceptedRewriteSuggestions`。
- [ ] 报告包含 `pendingRewriteSuggestions`。
- [ ] 报告包含 `nextStepChecklist`。
- [ ] 报告包含 `modelInfo`，聚合相关 AI 结果的模型名称和 Prompt 版本。
- [ ] 报告包含 `generatedAt`。
- [ ] 报告包含 `warnings`。
- [ ] 明确字段为空时的返回策略。
- [ ] 明确没有匹配结果、没有优化建议、没有局部改写时的提示策略。
...
- 新增报告聚合 Service。
- 从已有 AI 匹配结果、优化建议结果和局部改写建议中聚合报告内容。
- 保证用户只能查看自己的简历、目标岗位和报告结果。

任务：

- [ ] 创建 `JobOptimizationReportService`。
- [ ] 创建 `JobOptimizationReportServiceImpl`。
- [ ] 根据 `resumeId` 和 `jobDescriptionId` 查询当前用户简历。
- [ ] 校验简历属于当前用户。
- [ ] 查询目标岗位描述。
- [ ] 校验目标岗位描述属于当前用户。
- [ ] 查询对应的 AI 匹配结果。
- [ ] 若 AI 匹配结果不存在，返回明确业务错误。
- [ ] 若 AI 匹配结果不是 SUCCESS，返回明确提示。
- [ ] 查询对应的岗位优化建议结果。
- [ ] 若优化建议不存在，报告中展示空建议和提示。
- [ ] 查询对应的局部改写建议。
- [ ] 按采纳状态区分已采纳、未处理、已拒绝改写建议。
- [ ] 从 AI 匹配结果中提取匹配分数。
- [ ] 从 AI 匹配结果中提取强匹配项。
- [ ] 从 AI 匹配结果中提取弱匹配项。
- [ ] 从 AI 匹配结果中提取缺失技能。
- [ ] 从 AI 匹配结果中提取风险提示。
- [ ] 从优化建议结果中提取建议类型、优先级、目标章节、问题、建议、依据和注意事项。
- [ ] 从局部改写结果中提取原文、改写建议、改写理由、注意事项和采纳状态。
- [ ] 生成 `nextStepChecklist`。
- [ ] 聚合模型名称和 Prompt 版本。
- [ ] 生成报告 warnings。
- [ ] 避免返回完整 Prompt。
- [ ] 避免返回 API Key、Token 等敏感信息。
- [ ] 补充 Service 单元测试。

建议 `nextStepChecklist` 生成规则：

| 场景 | 待办 |
|---|---|
| 存在高优先级建议 | 优先处理高优先级岗位优化建议 |
| 存在缺失技能 | 检查是否真实掌握缺失技能，掌握则补充到技能或项目描述中 |
| 存在弱匹配项 | 优化对应经历表达，突出岗位相关内容 |
| 存在局部改写建议 | 逐条确认是否采纳改写建议 |
| 没有局部改写建议 | 可选择关键项目经历生成局部改写建议 |
| 存在风险提示 | 避免在简历中伪造未发生的经历或量化指标 |

验收标准：

- 可以正确聚合 AI 匹配、优化建议和局部改写结果。
- 用户只能查看自己的报告。
- 缺少优化建议或局部改写时不影响报告生成。
- 缺少成功 AI 匹配结果时给出明确错误。
- 不新增 AI 调用。
- 不新增数据库表。
- 单元测试覆盖成功、无建议、无改写、无匹配结果、跨用户访问等场景。
...
- 提供后端接口给前端查询岗位优化报告。
- 保持接口风格与现有 `/api/resumes/{id}/ai-job-matches`、`/api/resumes/{id}/ai-suggestions`、`/api/resumes/{id}/rewrite-suggestions` 一致。

任务：

- [ ] 创建 `JobOptimizationReportController`。
- [ ] 新增 `GET /api/resumes/{resumeId}/job-optimization-report`。
- [ ] 使用 `jobDescriptionId` 作为必填查询参数。
- [ ] 校验 `resumeId` 为正整数。
- [ ] 校验 `jobDescriptionId` 为正整数。
- [ ] 获取当前登录用户。
- [ ] 调用 `JobOptimizationReportService`。
- [ ] 返回统一 `Result<JobOptimizationReportVO>`。
- [ ] 将接口加入 Spring Security 鉴权范围。
- [ ] 补充 OpenAPI / Swagger 注解。
- [ ] 补充 API 集成测试。
- [ ] 验证未登录访问返回 401。
- [ ] 验证跨用户访问返回业务错误或 403。
- [ ] 验证无匹配结果时返回明确错误。
...
```http
GET /api/resumes/{resumeId}/job-optimization-report?jobDescriptionId={jobDescriptionId}
```
...
- 在“匹配与优化”页面展示岗位优化报告。
- 让用户完成匹配分析和岗位优化建议后，可以看到一个聚合后的最终结果页或结果区。
- 页面只做轻量增强，不进行 Phase 5 级别的大规模 UI 重构。

任务：

- [ ] 新增前端 API 方法 `getJobOptimizationReport`。
- [ ] 新增 TypeScript 类型 `JobOptimizationReport`。
- [ ] 在 `AiJobMatchView.vue` 中增加“查看优化报告”按钮。
- [ ] 按当前选中的简历和目标岗位查询报告。
- [ ] 展示简历名称。
- [ ] 展示目标岗位名称。
- [ ] 展示匹配分数。
...
- [ ] 展示模型名称和 Prompt 版本。
- [ ] 展示生成时间。
- [ ] 无优化建议时显示“请先生成岗位优化建议”提示。
- [ ] 无局部改写建议时显示“可选择关键片段生成局部改写”提示。
- [ ] 无报告数据时显示明确空状态。
- [ ] 查询失败时显示统一错误提示。
- [ ] 报告内容默认结构化展示，避免大段 JSON 直接展示。
- [ ] 前端构建通过。
...
- 在岗位优化报告中尽量展示每条结论和建议的依据。
- 优先复用已有 AI 结果中的 `evidence`、`basis`、`reason`、`caution` 等字段。
- 如果已有结果缺少依据字段，则在报告中保留建议内容，但生成 warning。

任务：

- [ ] 检查 AI 匹配结果结构中是否已有匹配依据字段。
- [ ] 检查岗位优化建议结构中是否已有依据字段。
- [ ] 检查局部改写建议结构中是否已有改写理由和注意事项。
- [ ] 报告中展示匹配依据。
- [ ] 报告中展示优化建议依据。
- [ ] 报告中展示局部改写理由。
- [ ] 报告中展示注意事项。
...
- [ ] 不为了补依据重新调用 AI。
- [ ] 不编造依据。
- [ ] 更新后续 Prompt 优化建议文档，可选。

建议 warning：

| 场景 | warning |
|---|---|
| 匹配结果缺少依据 | 当前匹配结果缺少详细依据，建议重新生成匹配分析 |
| 优化建议缺少依据 | 部分优化建议缺少依据，使用前请结合原简历确认 |
| 改写建议缺少注意事项 | 部分改写建议缺少注意事项，请确认没有新增虚假事实 |
...
## v2.10.6 README 展示入口、演示流程文档与项目主线文档

状态：未开始

目标：

- 为 GitHub 展示提前优化 README 的项目主线入口。
- 为后续部署和面试展示提前准备演示流程。
- 让项目主线更清晰，避免面试时功能很多但讲不清楚。
- 不等 Phase 5 才开始整理演示思路。

任务：

- [ ] 更新根目录 `README.md`。
- [ ] 在 `README.md` 中增加项目一句话介绍。
- [ ] 在 `README.md` 中增加项目核心价值。
- [ ] 在 `README.md` 中增加核心功能流程图或文字流程。
- [ ] 在 `README.md` 中增加当前已完成能力概览。
- [ ] 在 `README.md` 中增加技术栈概览。
- [ ] 在 `README.md` 中增加 AI 能力说明。
- [ ] 在 `README.md` 中增加演示流程入口。
- [ ] 在 `README.md` 中增加文档导航入口。
- [ ] 在 `README.md` 中链接到 `docs/project-storyline.md`。
- [ ] 在 `README.md` 中链接到 `docs/demo/demo-flow.md`。
- [ ] 创建 `docs/project-storyline.md`。
- [ ] 创建 `docs/demo/` 目录。
- [ ] 创建 `docs/demo/demo-flow.md`。
- [ ] 创建 `docs/demo/demo-resume-java-backend.md`。
- [ ] 创建 `docs/demo/demo-job-java-backend.md`。
- [ ] 在 `project-storyline.md` 中说明项目背景。
- [ ] 在 `project-storyline.md` 中说明用户痛点。
- [ ] 在 `project-storyline.md` 中说明系统解决方案。
- [ ] 在 `project-storyline.md` 中说明 Phase 1 到 Phase 3 的技术演进。
- [ ] 在 `project-storyline.md` 中说明后续 Phase 4 / Phase 5 规划。
- [ ] 在 `demo-flow.md` 中记录完整演示流程。
- [ ] 演示流程包含注册 / 登录。
- [ ] 演示流程包含上传简历。
- [ ] 演示流程包含解析简历。
- [ ] 演示流程包含简历诊断。
...
- 完成 v2.10 的后端、前端、文档联调和收口。
- 确认岗位优化报告可以作为 v2.10 的最终可演示产出。

任务：

- [ ] 使用一份已解析成功的简历测试报告。
- [ ] 使用一个已解析成功的目标岗位测试报告。
- [ ] 测试有 AI 匹配结果、有优化建议、有局部改写建议的完整场景。
- [ ] 测试有 AI 匹配结果、无优化建议的场景。
- [ ] 测试有 AI 匹配结果、无局部改写建议的场景。
- [ ] 测试无 AI 匹配结果的错误提示。
- [ ] 测试 AI 匹配结果失败状态的错误提示。
- [ ] 测试跨用户访问报告。
- [ ] 测试未登录访问报告。
...
- [ ] 执行后端测试。
- [ ] 执行前端构建。
- [ ] 检查报告不展示 API Key、Token、完整 Prompt 等敏感信息。
- [ ] 检查报告不自动编造简历内容。
- [ ] 更新 `docs/tasks/phase-3-task-list.md`。
- [ ] 创建 `docs/iteration-log/v2.10-optimization-report.md`。
- [ ] 在迭代日志中记录完成内容。
- [ ] 在迭代日志中记录当前不足。
- [ ] 在迭代日志中记录下一步进入 Phase 4 或继续收口的建议。
...
- 用户可以基于简历和目标岗位查看岗位优化报告。
- 报告聚合 AI 匹配结果、岗位优化建议和局部改写建议。
- 报告包含匹配分数、强匹配项、弱匹配项、缺失技能、风险提示。
- 报告包含按优先级分组的优化建议。
- 报告包含局部改写建议和采纳状态。
- 报告包含下一步修改清单。
- 报告尽量展示依据、理由和注意事项。
- 缺少依据时提示用户确认。
- 报告不编造任何用户不存在的经历、技能、证书、奖项或量化指标。
- 报告不自动修改原始简历。
- 报告不导出 DOCX / PDF。
- 前端可以结构化展示报告。
- 演示流程文档已建立。
