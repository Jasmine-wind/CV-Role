...
- 明确各模块职责边界。
- 轻量移动明显放错位置的类。
- 梳理 Controller / Service / Mapper / DTO / VO / Entity 的职责。
- 将基础设施能力与业务模块分离。
- 更新架构文档和迭代日志。
...
- 业务模块按领域聚合。
- 基础设施能力从业务模块中抽离。
- Controller 只做入口，不承担业务编排细节。
- Service 只做业务编排，不直接写超长 Prompt。
- Entity 不直接返回前端。
- DTO / VO / Entity 边界要清晰。
- 每一步修改后都要能编译通过。
...
确认当前项目后端包结构，并给出目标包结构规划。该小任务只做审查和文档确认，不强制移动代码。
...
| 包 | 职责 |
|---|---|
| `common` | 通用结果、异常、常量、工具类 |
| `config` | Spring 配置、安全配置、Web 配置、AI 配置、存储配置 |
| `infra.ai` | AI 客户端、Prompt 加载、AI 响应解析等基础能力 |
| `infra.storage` | 本地文件存储、对象存储适配 |
| `infra.embedding` | Embedding API 客户端、向量生成基础能力 |
| `infra.vector` | pgvector / 向量查询基础设施 |
| `module.auth` | 登录、注册、JWT、认证相关业务 |
| `module.user` | 用户资料、当前用户信息 |
| `module.resume` | 简历记录、上传、解析、展示结果 |
| `module.resume.parse` | 文本提取、清洗、章节识别、结构化解析 |
| `module.resume.display` | 简历解析结果展示模型构建 |
| `module.job` | 岗位库、目标岗位、岗位描述解析 |
| `module.analysis.diagnosis` | 简历诊断 |
| `module.analysis.match` | 简历与目标岗位匹配分析 |
| `module.analysis.suggestion` | 岗位优化建议 |
| `module.analysis.rewrite` | 局部改写建议 |
| `module.analysis.report` | 岗位优化报告聚合 |
| `module.embedding` | 简历 / 岗位向量业务编排 |
| `module.history` | AI 结果回看、历史聚合查询 |

## 任务

- [ ] 扫描当前 `backend/src/main/java/com/winter/airesumeoptimizer` 包结构。
- [ ] 列出当前已有一级包和二级包。
- [ ] 标记当前包结构中明显职责不清的位置。
- [ ] 标记当前不建议移动的位置。
- [ ] 明确目标包结构。
- [ ] 明确哪些包当前保留不动。
- [ ] 明确哪些包后续可以小步整理。
- [ ] 更新 `docs/codex-context.md` 中的后端包结构说明。
- [ ] 更新 `docs/iteration-log/v3.2-package-structure.md`。
...
围绕用户、简历、岗位、AI 分析、历史、向量等领域边界，整理明显放错位置的类，使模块职责更清楚。
...
```text
岗位库：系统预置岗位，用于演示和参考。
目标岗位：用户粘贴真实 JD，用于匹配和优化。
```

建议边界：

- `job.preset`：系统预置岗位。
- `job.description`：用户粘贴的目标岗位 JD。
- `job.parse`：目标岗位解析。

岗位描述应支持来源类型：

```text
USER_INPUT：用户粘贴岗位 JD
PRESET：系统预置岗位
CRAWLED：爬虫来源，当前不实现，只预留语义
```
...
- 把岗位库做成管理员后台。
- 岗位描述解析直接生成简历改写文本。
- 岗位库和用户粘贴 JD 在前端语义上混用。

---

### 4. AI 分析模块

`analysis` 只负责“已存在简历 + 已存在目标岗位”的 AI 分析能力。
...
| 子模块 | 职责 | 禁止 |
|---|---|---|
| diagnosis | 只分析简历本身质量 | 不绑定目标岗位，不生成岗位建议 |
| match | 分析简历与目标岗位匹配关系 | 不生成改写文本 |
| suggestion | 基于匹配差距生成优化建议 | 不直接改写简历原文 |
| rewrite | 对用户选中片段生成改写版本 | 不重新做岗位匹配 |
| report | 聚合已有结果形成报告视图 | 不重新调用 AI |
...
- 用 Embedding 代替基础字段解析。
- 用 RAG 识别姓名、手机号、邮箱等精确字段。
- 在解析结构不稳定时强行生成大量低质量 chunk。
...
- [ ] 标记职责混杂的类。
- [ ] 标记明显应该移动到 `infra` 的类。
- [ ] 标记明显应该移动到 `resume.parse` 的类。
- [ ] 标记明显应该移动到 `resume.display` 的类。
- [ ] 标记明显应该移动到 `analysis.*` 子包的类。
- [ ] 标记当前不建议移动的大类。
- [ ] 小范围移动低风险类。
- [ ] 更新 import。
- [ ] 保证包路径修改后编译通过。
...
- 超过 1000 行且牵涉范围大的核心 Service。
- 与多个 Controller 强耦合的历史遗留类。
- 会影响接口路径或序列化字段的类。
- 会导致大量测试失效的类。
...
检查并轻量整理 Controller / Service 职责，避免 Controller 承担业务编排，避免 Service 混入过多 Prompt、AI 调用细节、VO 组装和展示逻辑。

## 职责边界

| 层级 | 应该做什么 | 不应该做什么 |
|---|---|---|
| Controller | 接收请求、获取当前用户、参数校验、调用 Service、返回 Result/VO | 不拼 Prompt，不直接操作数据库，不写复杂业务逻辑 |
| Service | 业务编排、权限校验、状态流转、调用 Mapper / 组件 | 不堆所有功能，不写超长 Prompt，不做前端复杂展示逻辑 |
| Assembler | Entity / JSON / 业务对象转 VO | 不访问数据库，不调用 AI |
| PromptService | 生成 Prompt、管理版本 | 不访问数据库，不编排业务 |
| AiClient | 统一调用模型 | 不理解具体业务语义 |
| ResultParser | 解析 AI 返回 JSON | 不做业务流程判断 |
| DisplayBuilder | 构造前端展示模型 | 不修改 structuredData 原始事实 |
...
- `AiJobMatchController`
- `HistoryController`
- `ResumeServiceImpl`
- `ResumeParseServiceImpl`
- `ResumeDisplayModelServiceImpl`
- `ResumeStructuredResultAssembler`
- `AiJobMatchServiceImpl`
- `AiResumeSuggestionServiceImpl`
- `AiRewriteSuggestionServiceImpl`
- `JobOptimizationReportServiceImpl`
- `AiHistoryServiceImpl`
...
- `ResumeParseValidator`

---

### 3. 报告 Service 检查

`JobOptimizationReportService` 应只聚合已有结果。
...
- 不改变历史记录。
- 可以聚合多个结果表。
- 返回 resultType / displayName / summary / updatedAt。

## 任务

- [ ] 扫描重点 Controller。
- [ ] 标记 Controller 中不应出现的业务逻辑。
- [ ] 扫描重点 Service。
- [ ] 标记 Service 中过长私有方法。
- [ ] 标记 Prompt 拼接位置。
- [ ] 标记 AI JSON 解析位置。
- [ ] 标记 VO 转换位置。
- [ ] 将低风险 VO 转换移动到 Assembler。
- [ ] 将低风险 Prompt 构造移动到 PromptService。
- [ ] 将低风险展示构造移动到 DisplayBuilder。
- [ ] 保持接口路径不变。
- [ ] 保持返回字段语义不变。
- [ ] 运行后端构建检查。
- [ ] 更新文档和日志。
...
将 AI 客户端、Embedding 客户端、文件存储、pgvector、配置属性等基础设施能力与业务模块解耦，使业务模块只依赖接口，不依赖具体实现细节。
...
- 在 AI Client 中写“简历诊断”“岗位匹配”等业务判断。
- 在 AI Client 中直接访问 Mapper。
- 在 AI Client 中拼具体业务 Prompt。
...
- 在 Embedding Client 中决定业务 chunk 如何切分。
- 在 Embedding Client 中直接操作简历业务表。
- 用 Embedding Client 代替字段解析器。

---

### 4. Vector / pgvector 基础设施
...
- 在 vector 基础设施中写岗位匹配业务结论。
- 在 vector 基础设施中调用 AI 建议生成。
...
- [ ] 扫描 AI Client 相关类。
- [ ] 扫描 Prompt 加载相关类。
- [ ] 扫描 Embedding Client 相关类。
- [ ] 扫描文件存储相关类。
- [ ] 扫描 pgvector / vector 查询相关类。
- [ ] 标记基础设施类当前所在位置。
- [ ] 标记明显应迁移到 `infra` 的类。
- [ ] 低风险移动基础设施类。
- [ ] 为业务层保留接口依赖。
...
- AI Client 和具体业务 Prompt 解耦。
- Embedding Client 和业务 chunk 生成解耦。
- 文件存储实现和简历业务权限解耦。
- 业务 Service 依赖接口，不强依赖具体实现类。
- Spring Bean 正常加载。
- 后端编译通过。
- 不改业务行为。
...
完成 v3.2 包结构整理后的编译、接口、前后端手动流程验证，并记录本轮架构优化结果。
...
| 接口路径未改变 |  |
| 数据库结构未改变 |  |
| Controller 未新增复杂业务逻辑 |  |
| 报告接口未重新调用 AI |  |
| 历史接口未触发新 AI 生成 |  |
| AI Client 未混入具体业务逻辑 |  |
| Embedding Client 未混入字段解析逻辑 |  |
| 文件存储未混入简历解析逻辑 |  |
| 前端主流程未被破坏 |  |
| 文档已更新 |  |
...
- [ ] 新增或更新 `docs/iteration-log/v3.2-package-structure.md`。
- [ ] 如有必要，更新 `docs/project-structure.md`。
- [ ] 如有必要，更新 `docs/product-structure.md`。
- [ ] 记录移动过的类。
- [ ] 记录没有移动但后续建议拆分的类。
- [ ] 记录验证命令和结果。
- [ ] 记录遗留风险。
